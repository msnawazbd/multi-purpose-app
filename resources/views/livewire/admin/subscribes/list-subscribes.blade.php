                            ...
                            <table class="table table-hover text-nowrap">
                                ...
                                @forelse($subscribes as $key => $subscribe)
                                    <tr>
                                        ...
                                        <td class="align-middle">
                                            <span class="badge badge-{{ $subscribe->status == 1 ? 'success' : 'warning' }}">
                                                {{ $subscribe->status == 1 ? 'PUBLISHED' : 'UNPUBLISHED' }}
                                            </span>
                                        </td>
                                            ...
                                            <div class="btn-group btn-group-sm">
                                                ...
                                                <div class="dropdown-menu" role="menu" style="">
                                                    <button class="dropdown-item"
                                                            wire:click.prevent="changeStatus({{ $subscribe->id }})">
                                                        <i class="fas fa-{{ $subscribe->status == 1 ? 'arrow-down' : 'arrow-up' }} mr-2"></i>
                                                        {{ $subscribe->status == 1 ? 'Unpublished' : 'Published' }}
                                                    </button>
                                                    <div class="dropdown-divider"></div>
                                                    <button class="dropdown-item"
                                                            wire:click.prevent="destroy({{ $subscribe->id }})"><i
