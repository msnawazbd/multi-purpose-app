                    ...
                    <div class="card">
                        ...
                        <div class="card-body table-responsive">
                            <table class="table table-hover text-nowrap">
                                ...
                                @forelse($roles as $key => $v_role)
                                    <tr>
                                        <td class="align-middle">{{ $roles->firstItem() + $key }}</td>
                                        <td class="align-middle"><span class="text-capitalize">{{ $v_role->name }}</span></td>
                                        <td class="align-middle">{{ $v_role->guard_name }}</td>
                                        <td class="align-middle">
                                            @foreach($v_role->permissions as $permission)
                                                <a style="cursor: pointer;"
                                                   wire:click.prevent="revokePermission({{ $v_role }}, '{{ $permission->name }}')">
                                        ...
                                        <td class="align-middle">
                                            {{ $v_role->created_at ? $v_role->created_at->toFormattedDate() : 'N/A' }}
                                        </td>
                                        <td class="align-middle">
                                            <span
                                                class="badge badge-{{ $v_role->status == 1 ? 'success' : 'warning' }}">
                                                {{ $v_role->status == 1 ? 'PUBLISHED' : 'UNPUBLISHED' }}
                                            </span>
                                        </td>
                                            ...
                                            <div class="btn-group btn-group-sm">
                                                ...
                                                <div class="dropdown-menu" role="menu" style="">
                                                    <button class="dropdown-item"
                                                            wire:click.prevent="changeStatus({{ $v_role->id }})">
                                                        <i class="fas fa-{{ $v_role->status == 1 ? 'arrow-down' : 'arrow-up' }} mr-2"></i>
                                                        {{ $v_role->status == 1 ? 'Unpublished' : 'Published' }}
                                                    </button>
                                                    <div class="dropdown-divider"></div>
                                                    <button class="dropdown-item"
                                                            wire:click.prevent="show({{ $v_role->id }})">
                                                        <i class="fas fa-eye mr-2"></i> View
                                                    </button>
                                                    <button class="dropdown-item"
                                                            wire:click.prevent="edit({{ $v_role }})">
                                                        <i class="fas fa-edit mr-2"></i> Edit
                                                    </button>
                                                    <button class="dropdown-item"
                                                            wire:click.prevent="givePermission({{ $v_role->id }})">
                                                        <i class="fas fa-unlock mr-2"></i> Give Permissions
                                                    </button>
                                                    <div class="dropdown-divider"></div>
                                                    <button class="dropdown-item"
                                                            wire:click.prevent="destroy({{ $v_role->id }})"><i
                ...
                <div class="modal-body pt-0">
                    <table class="table">
                        ...
                        <tr>
                            ...
                            <td><span class="text-capitalize">{{ $name }}</span></td>
                        </tr>
                        <tr>
        ...
        <div class="modal fade" id="permissionModal" tabindex="-1" aria-labelledby="cu-form-label" aria-hidden="true"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">
                            <span class="text-capitalize">Give Permission To {{ $role->name }}</span>
                        </h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="pb-2 mb-2">
                                @foreach($role->permissions as $permission)
                                    <a style="cursor: pointer"
                                       wire:click.prevent="revokePermission({{ $role }}, '{{ $permission->name }}')">
